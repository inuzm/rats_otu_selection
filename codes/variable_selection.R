...
option_list = list(
    ...
    make_option(c("-l", "--level"), type = "character", default = "genus", 
                ...
                metavar = "character"),
    make_option(c("-M", "--metadata"), type = "character", default = "../metadata/variables_desenlace.xlsx", 
...
pacman::p_load(ggplot2, ggthemes,                       # Plots
               dplyr, tibble, tidyr, purrr, broom, pscl, readxl)                 # Data frame manipulation 
# library(ggplot2)
# library(ggthemes)
# library(dplyr)
# library(tibble)
...
differentialOtusPvalues <- function(db, model) {
    ...
    pValues <- data.frame(
        ...
        score = double(), g1 = character(), 
        g2 = character(), g1vsg2 = character(), adj_pvalues = double()
    )
...
computePvaluesLevel <- function(hLevel, path_to_counts, path_to_metadata, 
    ...
    mdb <- read_excel(path_to_metadata) %>% 
        rename(Sample = muestra)
    db <- left_join(db, mdb, by = 'Sample') %>% 
        select(c('OTU', 'Sample', 'grupo', 'Abundance')) %>% 
        rename(Group = grupo)
...
variableSelection <- function(hlevels = c("phylum", "order", "genus"), 
                              path_to_counts, path_to_metadata, kpvalues = 5, 
                              model = "nb") {
